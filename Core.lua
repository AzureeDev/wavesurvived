...
Hooks:Add("MenuManagerOnOpenMenu", "WaveSurvived_Alert_Compatibility", function( menu_manager, menu, position )
	if menu == "menu_main" then
		...
		function WaveSurvived:Compatibility_Alert_Check()
			...
			if hud_lddg and WaveSurvived.options["WaveSurvived_compatibility"] ~= 4 then
			...
			if restoration and restoration.Options then
				if restoration.Options:GetValue("HUD/AssaultPanel") and WaveSurvived.options["WaveSurvived_compatibility"] ~= 5 then
					WaveSurvived:Compatibility_Alert_Message()
					local hud_number = 5
					WaveSurvived.options.WaveSurvived_compatibility = hud_number
					log("[WaveSurvived] Auto-changed the HUD : " .. WaveSurvived.options.WaveSurvived_compatibility)
			...
			if pdth_hud and pdth_hud.Options then
				if pdth_hud.Options:GetValue("HUD/Assault") and WaveSurvived.options["WaveSurvived_compatibility"] ~= 3 then
					WaveSurvived:Compatibility_Alert_Message()
					local hud_number = 3
					WaveSurvived.options.WaveSurvived_compatibility = hud_number
					log("[WaveSurvived] Auto-changed the HUD : " .. WaveSurvived.options.WaveSurvived_compatibility)
			...
			if Holo and Holo.Options then 
				if Holo.Options:GetValue("HudBox") and Holo:ShouldModify("Hud", "HudAssault") and WaveSurvived.options["WaveSurvived_compatibility"] ~= 2 then
					WaveSurvived:Compatibility_Alert_Message()
					local hud_number = 2
					WaveSurvived.options.WaveSurvived_compatibility = hud_number
					log("[WaveSurvived] Auto-changed the HUD : " .. WaveSurvived.options.WaveSurvived_compatibility)
			...
			if not pdth_hud and not hud_lddg and not restoration and not Holo and WaveSurvived.options["WaveSurvived_compatibility"] ~= 1 then
				WaveSurvived:Compatibility_Alert_Message()
				local hud_number = 1
				WaveSurvived.options.WaveSurvived_compatibility = hud_number
				log("[WaveSurvived] Auto-changed the HUD : " .. WaveSurvived.options.WaveSurvived_compatibility)
